...
class SimpleAudioDataset(Dataset):
    ...
    def combine_with_noise(self, audio_file):
        ...
        temp_folder = "temp_files_conformer"
...
if __name__ == '__main__':
    real_real_dir = './combined_audio2_conformer/real_real'
    real_fake_dir = './combined_audio2_conformer/real_fake'
    fake_fake_dir = './combined_audio2_conformer/fake_fake'
    ...
    model = FrontendDeepVoiceConformer(fc1_dim=model_config['fc1_dim'],
...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, device, epochs, save_path, interval=3):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(save_path, f'best_model_conformer{epoch+1}.pth')
            torch.save(model.state_dict(), best_model_path)
            print(f"Best model saved at {best_model_path}")
        
        if (epoch + 1) % interval == 0:
            interval_model_path = os.path.join(save_path, f"model_epoch_{epoch+1}_conformer.pth")
...
save_path = 'checkpoints_conformer'
